...
func (m *Window) ConfigSigAt(gwName string, t time.Time) (uint64, bool) {
	...
	for i := range m.w.VSes {
		r := &m.w.VSes[i]
		if asOf(r.ValidFrom, r.ValidTo, t) && boundTo(r.Namespace, r.BoundGateways, gw.Namespace, gwName) {
...
// boundTo reports whether a VirtualService in vsNS with spec.gateways values
// bound references the gateway gwNS/gwName. bound carries the RAW spec.gateways
// strings (the production ingest does not normalize them), and Istio accepts two
// forms: the qualified "<ns>/<name>", and the bare "<name>" which binds to a
// gateway in the VS's OWN namespace.
