...
import (
	...
	"github.com/example/metadata-exporter/test/integration/routesim/internal/gwresolve"
	"github.com/example/metadata-exporter/test/integration/routesim/internal/matchcheck"
	"github.com/example/metadata-exporter/test/integration/routesim/internal/memwindow"
...
// WindowStore is the one store capability the range query needs: the scoped
// Overlap load. *chstore.Store satisfies it.
type WindowStore interface {
	LoadTrafficWindow(ctx context.Context, ip string, t0, t1 time.Time) (store.TrafficWindow, error)
}
...
func (d Deps) Resolve(ctx context.Context, st WindowStore, host, path, ip string, port int, t0, t1 time.Time) ([]VersionResolution, error) {
...
func (d Deps) ResolveTimed(ctx context.Context, st WindowStore, host, path, ip string, port int, t0, t1 time.Time) ([]VersionResolution, *Metrics, error) {
	m := &Metrics{}
